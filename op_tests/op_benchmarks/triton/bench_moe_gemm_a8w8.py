...
from aiter.ops.triton.moe_op_gemm_a8w8 import moe_gemm_a8w8, swizzle_scales, downcast_to_static_fp8
...
def bench_mlp(batch, dim1, dim2, n_expts_tot, n_expts_act, x_dtype, w_dtype, TP, op_regex):
    ...
    for i in range(reps):
        ...
        if x_dtype_str == "fp8":
            ...
            x = moe_gemm_a8w8(x, w1, None, w1_scale, static_scale, static_scale, b1, rdata, gather_indx=gather_indx, swizzle_mx_scale=swizzle_mx_scale1, out_dtype=x_dtype, apply_swiglu=True)
            x = moe_gemm_a8w8(x, w2, None, w2_scale, static_scale, None, b2, rdata, scatter_indx=scatter_indx, swizzle_mx_scale=swizzle_mx_scale2)
        else:
            assert x_dtype_str == "mx8"
            x, _, x_scale = quantize(x, x_dtype_str)
            x = moe_gemm_a8w8(x, w1, x_scale, w1_scale, None, None, b1, rdata, gather_indx=gather_indx, swizzle_mx_scale="CDNA4_SCALE", apply_swiglu=True)
            x, _, x_scale = quantize(x, x_dtype_str)
            x = moe_gemm_a8w8(x, w2, x_scale, w2_scale, None, None, b2, rdata, scatter_indx=scatter_indx, swizzle_mx_scale="CDNA4_SCALE")
    proton.finalize()
...
if __name__ == "__main__":
    ...
    quantized_dtypes = [args.act_dtype, "mx8"] 
